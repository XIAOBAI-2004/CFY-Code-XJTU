 ...
 *   @note      STM480MC的命令发送,也是用该函数实现(不带参数的命令, 也会有一个状态寄存器需要读取)
 * @param       addr  : 寄存器地址/命令
 * @param       length: 读取长度
 * @param       buf   : 数据存储buf
 * @retval      0, 操作成功
...
uint8_t st480mc_read_nbytes(uint8_t addr, uint8_t length, uint8_t *buf)
{
    ...
    iic_start();
    iic_send_byte(ST480MC_ADDR | 0X00); /* IIC地址最低位是0, 表示写入 */
    ...
    iic_start();
    iic_send_byte(ST480MC_ADDR | 0x01); /* IIC地址最低位是1, 表示读取 */
    iic_wait_ack();
...
uint8_t st480mc_read_magdata(int16_t *pmagx, int16_t *pmagy, int16_t *pmagz)
{
    ...
    st480mc_read_nbytes(ST480MC_SINGLR_MODE & 0XFE, 1, buf);    /* 发送单次测量命令(不测量温度) */
