 ...
 * 第一次发布
 * V1.1 20221126
 * 1,去掉全局数组,将数据读写方式改成HAL库版本形式,以提高读写性能,注意:改成这种方式以后,不能直接
 *   从SD卡读取数据写入外部SRAM等访问速度较慢的RAM，否则可能产生FIFO上溢错误！可以采用加BUFFER
 *   的形式，先读取到内部SRAM，然后再拷贝到外部SRAM等访问速度较慢的RAM里面
 * 2,修改sd_read_disk/sd_write_disk/sdmmc_read_blocks/sdmmc_write_blocks等4个函数
 *
 ****************************************************************************************************
 */
...
SD_Error sd_init(void)
{
    ...
    if ((errorstatus == SD_OK) || (MULTIMEDIA_CARD == CardType))
    {
        ...
        /* 设置时钟频率,SDIO时钟计算公式: SDIO_CK 时钟 = SDIOCLK / [2 + clkdiv]; 其中, SDIOCLK 一般为48Mhz */
