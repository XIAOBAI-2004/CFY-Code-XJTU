...
void dcmi_dma_init(uint32_t mem0addr, uint32_t mem1addr, uint16_t memsize, uint8_t memblen, uint8_t meminc)
{
    ...
    while (DMA2_Stream1->CR & 0X01);    /* 等待DMA2可配置 */

    DMA2->LIFCR |= 0X3D << 6 * 1;       /* 清空通道1上所有中断标志 */
    DMA2_Stream1->FCR = 0X0000021;      /* 设置为默认值 */

    DMA2_Stream1->PAR = (uint32_t)&DCMI->DR; /* 外设地址为:DCMI->DR */
    DMA2_Stream1->M0AR = mem0addr;      /* memaddr作为目标地址 */
    DMA2_Stream1->M1AR = mem1addr;      /* memaddr作为目标地址 */
    ...
    tempreg |= 1 << 25;                 /* 选择:通道1 DCMI通道 */

    if (mem1addr)   /* 双缓冲的时候,才需要开启 */
    {
        tempreg |= 1 << 18;             /* 双缓冲模式 */
        tempreg |= 1 << 4;              /* 开启传输完成中断 */
        sys_nvic_init(2, 3, DMA2_Stream1_IRQn, 2);  /* 抢占1，子优先级3，组2 */
    }
