...
void HAL_HCD_MspInit(HCD_HandleTypeDef *hhcd)
{
    if (hhcd->Instance == USB_OTG_FS)
    {
        ...
        sys_gpio_set(GPIOA, SYS_GPIO_PIN15,
                     SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_NONE);/* PA15引脚模式设置,推挽输出 */

        sys_gpio_af_set(GPIOA, SYS_GPIO_PIN11, 10);     /* PA11, AF10(USB) */
        sys_gpio_af_set(GPIOA, SYS_GPIO_PIN12, 10);     /* PA12, AF10(USB) */
        
        sys_gpio_pin_set(GPIOA, SYS_GPIO_PIN15, 0);     /* PA15输出低电平, 关闭U盘供电 */
        delay_ms(500);
        sys_gpio_pin_set(GPIOA, SYS_GPIO_PIN15, 1);     /* PA15输出高电平, 恢复U盘供电 */
...
{
    printf("USBH_LL_OpenPipe%d,%d,%d,%d,%d,%d\r\n",pipe,epnum,dev_address,speed,ep_type,mps);
