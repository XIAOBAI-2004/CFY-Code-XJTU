...
void filelist_delete(_filelistbox_obj *filelistbox)
{
	...
	if (g_active_flistbox == filelistbox) {
        g_active_flistbox = NULL; // 离开文件列表时强制置空
        printf("[STATE] Filelist closed, reset g_active_flistbox\n");
    }
...
// filelistbox.c 新增
/* 根据UTF8文件名获取全局索引（跳过文件夹） */
int16_t audio_get_global_index(_filelistbox_obj *flist, const uint8_t *name)
{
    _filelistbox_list *p = flist->list;
    int16_t index = 0;
    
    while(p) {
        // 跳过文件夹条目
        if(p->type != FICO_FOLDER && p->type != FICO_DISK) {
            if(strcmp((char*)p->name, (char*)name) == 0) {
                return index - flist->foldercnt; // 返回文件索引
            }
        }
        p = p->nextlist;
        index++;
...
int16_t audio_get_global_index11(_filelistbox_obj *flist, const uint8_t *name) {
    /* 1. 字符串转整数 */
    char *endptr;
    long target_index = strtol((char*)name, &endptr, 10);
    
    /* 2. 验证转换有效性 */
    if (endptr == (char*)name || *endptr != '\0') {
        u3_printf("Error: Invalid index format: %s\r\n", name);
        return -1;
    }
    
    /* 3. 检查索引范围 */
    if (target_index < 0 || target_index >= flist->filecnt) {
        u3_printf("Error: Index %ld out of range (0-%d)\r\n", 
                  target_index, flist->filecnt - 1);
        return -1;
    }
    ...
    while (p) {
        if (p->type != FICO_FOLDER && p->type != FICO_DISK) {
            if (file_count == target_index) {
                /* 5. 返回全局索引（总条目索引 - 文件夹数量） */
                return file_count + flist->foldercnt;
            }
            file_count++;
        }
        p = p->nextlist;
    ...
    return -1;  // 不应执行到此处（前面已检查范围）
}



/* 确保目标项在可视区域内 */
void filelistbox_ensure_visible(_filelistbox_obj *flist, uint16_t index) {
    // 计算可见范围
    uint16_t top = flist->scbv->topitem;
    uint16_t bottom = top + flist->scbv->itemsperpage - 1;

    if(index < top) {  // 目标在可视区域上方
        flist->scbv->topitem = index;
    } else if(index > bottom) {  // 目标在可视区域下方
        flist->scbv->topitem = index - flist->scbv->itemsperpage + 1;
    }

    // 边界保护
    if(flist->scbv->topitem + flist->scbv->itemsperpage > flist->scbv->totalitems) {
        flist->scbv->topitem = flist->scbv->totalitems - flist->scbv->itemsperpage;
    }
    
    filelistbox_draw_listbox(flist);  // 重绘列表
}
