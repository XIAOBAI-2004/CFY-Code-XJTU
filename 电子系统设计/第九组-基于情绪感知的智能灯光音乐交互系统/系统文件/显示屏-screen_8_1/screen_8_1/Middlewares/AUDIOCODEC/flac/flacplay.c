...
void flac_i2s_dma_tx_callback(void)
{
    ...
    if (DMA1_Stream4->CR & (1 << 19))
    {
        ...
        if ((g_audiodev.status & 0X01) == 0) /* 暂停了,填充0 */
        {
            ...
            for (i = 0; i < size; i++)g_audiodev.i2sbuf1[i] = 0;
        }
    }
    else
    {
        ...
        if ((g_audiodev.status & 0X01) == 0) /* 暂停了,填充0 */
        {
            ...
            for (i = 0; i < size; i++)g_audiodev.i2sbuf2[i] = 0;
        }
    }
...
uint8_t flac_play_song(uint8_t *fname)
{
    ...
    else
    {
        ...
        if (res == FR_OK)
        {
            ...
            if (fc->min_blocksize == fc->max_blocksize && fc->max_blocksize != 0) /* 必须min_blocksize等于max_blocksize */
            {
                ...
                {
                    g_audiodev.i2sbuf1 = mymalloc(SRAMIN, fc->max_blocksize * 8);
                    g_audiodev.i2sbuf2 = mymalloc(SRAMIN, fc->max_blocksize * 8);
                }
                else    /* 16位音频数据 */
                {
                    g_audiodev.i2sbuf1 = mymalloc(SRAMIN, fc->max_blocksize * 4);
                    g_audiodev.i2sbuf2 = mymalloc(SRAMIN, fc->max_blocksize * 4);
                }
    ...
    if (buffer && g_audiodev.i2sbuf1 && g_audiodev.i2sbuf2 && decbuf0 && decbuf1 && res == 0)
    {
        ...
        {
            es8388_i2s_cfg(0, 0);   /* 飞利浦标准,24位数据长度（同样的配置WM8978可以正常，ES8388 24bit异常，有待解决！） */
            i2s_init(0,2,0,2);      /* 飞利浦标准,主机发送,时钟低电平有效,24位扩展帧长度 */
            i2s_tx_dma_init(g_audiodev.i2sbuf1, g_audiodev.i2sbuf2, flacctrl->outsamples * 2);  /* 配置TX DMA */
            
            memset(g_audiodev.i2sbuf1, 0, fc->max_blocksize * 8);
            memset(g_audiodev.i2sbuf2, 0, fc->max_blocksize * 8);
        }
        else    /* 16位音频数据 */
        {
            es8388_i2s_cfg(0, 3);   /* 飞利浦标准,16位数据长度 */
            i2s_init(0,2,0,1);      /* 飞利浦标准,主机发送,时钟低电平有效,16位扩展帧长度 */
            i2s_tx_dma_init(g_audiodev.i2sbuf1, g_audiodev.i2sbuf2, flacctrl->outsamples);      /* 配置TX DMA */
            
            memset(g_audiodev.i2sbuf1, 0, fc->max_blocksize * 4);
            memset(g_audiodev.i2sbuf2, 0, fc->max_blocksize * 4);
        }

        i2s_samplerate_set(fc->samplerate); /* 设置采样率 */
        ...
        while (bytesleft)
        {
            ...
            if (flacwitchbuf == 0)p8 = g_audiodev.i2sbuf1;
            else p8 = g_audiodev.i2sbuf2;
    ...
    myfree(SRAMIN, g_audiodev.i2sbuf1);
    myfree(SRAMIN, g_audiodev.i2sbuf2);
