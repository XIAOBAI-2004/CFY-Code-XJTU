...
void mp3_i2s_dma_tx_callback(void)
{
    ...
    if (DMA1_Stream4->CR & (1 << 19))
    {
        ...
        {
            for (i = 0; i < 2304 * 2; i++)g_audiodev.i2sbuf1[i] = 0;
        }
    }
    else
    {
        ...
        if ((g_audiodev.status & 0X01) == 0) /* 暂停了,填充0 */
        {
            for (i = 0; i < 2304 * 2; i++)g_audiodev.i2sbuf2[i] = 0;
        }
    }
...
void mp3_fill_buffer(uint16_t *buf, uint16_t size, uint8_t nch)
{
    ...
    if (mp3witchbuf == 0)
    {
        p = (uint16_t *)g_audiodev.i2sbuf1;
    }
    else
    {
        p = (uint16_t *)g_audiodev.i2sbuf2;
    }
...
uint8_t mp3_play_song(uint8_t *fname)
{
    ...
    g_audiodev.i2sbuf1 = mymalloc(SRAMIN, 2304 * 2);
    g_audiodev.i2sbuf2 = mymalloc(SRAMIN, 2304 * 2);
    g_audiodev.tbuf = mymalloc(SRAMIN, 2304 * 2);
    g_audiodev.file_seek = mp3_file_seek;

    if (!mp3ctrl || !buffer || !g_audiodev.file || !g_audiodev.i2sbuf1 || !g_audiodev.i2sbuf2 || !g_audiodev.tbuf) /* 内存申请失败 */
    {
    ...
    memset(g_audiodev.i2sbuf1, 0, 2304 * 2);    /* 数据清零 */
    memset(g_audiodev.i2sbuf2, 0, 2304 * 2);    /* 数据清零 */
    memset(mp3ctrl, 0, sizeof(__mp3ctrl));      /* 数据清零 */
    res = mp3_get_info(fname, mp3ctrl);

		// 新增调试信息
    u3_printf("Final play path: %s\r\n", fname); 
    ...
    if (res == 0)
    {
        ...
        i2s_samplerate_set(mp3ctrl->samplerate);    /* 设置采样率 */
        i2s_tx_dma_init(g_audiodev.i2sbuf1, g_audiodev.i2sbuf2, mp3ctrl->outsamples);    /* 配置TX DMA */
        i2s_tx_callback = mp3_i2s_dma_tx_callback;  /* 回调函数指mp3_i2s_dma_tx_callback */

        mp3decoder = MP3InitDecoder();              /* MP3解码申请内存 */
			// 新增调试信息
    u3_printf("Attempting to open: %s\r\n", fname); 
        res = f_open(g_audiodev.file, (char *)fname, FA_READ);  /* 打开文件 */
			  
			  if(res != FR_OK) {
        u3_printf("Open failed! Path:%s Error:%d\r\n", fname, res);
        return AP_ERR;
    }
