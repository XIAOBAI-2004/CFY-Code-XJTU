        ...
        if (res == 0) /* 解析文件成功 */
        {
            ...
            if (res == 0)
            {
                ...
                while (1)
                {
                    ...
                    if (wavwitchbuf)fillnum = wav_buffill(g_audiodev.i2sbuf2, WAV_I2S_TX_DMA_BUFSIZE, wavctrl.bps); /* 填充buf2 */
                    else fillnum = wav_buffill(g_audiodev.i2sbuf1, WAV_I2S_TX_DMA_BUFSIZE, wavctrl.bps); /* 填充buf1 */
                    ...
                    if ((g_audiodev.status & (1 << 1)) == 0 || (fillnum != WAV_I2S_TX_DMA_BUFSIZE)) /* 请求结束播放/播放完成 */
                    {
                        break;
                    }
                }
