...
void ape_i2s_dma_tx_callback(void)
{
    ...
    if (DMA1_Stream4->CR & (1 << 19))
    {
        ...
        {
            for (i = 0; i < APE_BLOCKS_PER_LOOP * 4; i++)g_audiodev.i2sbuf1[i] = 0;
        }
    }
    else
    {
        ...
        if ((g_audiodev.status & 0X01) == 0) /* 暂停了,填充0 */
        {
            for (i = 0; i < APE_BLOCKS_PER_LOOP * 4; i++)g_audiodev.i2sbuf2[i] = 0;
        }
    }
...
void ape_fill_buffer(uint32_t *buf, uint16_t size)
{
    ...
    if (apewitchbuf == 0)
    {
        p = (uint32_t *)g_audiodev.i2sbuf1;
    }
    else
    {
        p = (uint32_t *)g_audiodev.i2sbuf2;
    }
...
uint8_t ape_play_song(uint8_t *fname)
{
    ...
    g_audiodev.i2sbuf1 = mymalloc(SRAMIN, APE_BLOCKS_PER_LOOP * 4);
    g_audiodev.i2sbuf2 = mymalloc(SRAMIN, APE_BLOCKS_PER_LOOP * 4);
    g_audiodev.file_seek = ape_file_seek;
    buffer = mymalloc(SRAMIN, APE_FILE_BUF_SZ);

    if (filterbuf64 && apectrl && apex && decoded0 && decoded1 && g_audiodev.file && g_audiodev.i2sbuf1 && g_audiodev.i2sbuf2 && buffer) /* 所有内存申请都OK */
    {
        memset(apex, 0, sizeof(struct ape_ctx_t));
        memset(apectrl, 0, sizeof(__apectrl));
        memset(g_audiodev.i2sbuf1, 0, APE_BLOCKS_PER_LOOP * 4);
        memset(g_audiodev.i2sbuf2, 0, APE_BLOCKS_PER_LOOP * 4);
    ...
    if (res == 0)
    {
        ...
        i2s_samplerate_set(apex->samplerate);       /* 设置采样率 */
        i2s_tx_dma_init(g_audiodev.i2sbuf1, g_audiodev.i2sbuf2, APE_BLOCKS_PER_LOOP * 2);    /* 配置TX DMA */
        i2s_tx_callback = ape_i2s_dma_tx_callback;  /* 回调函数指ape_i2s_dma_tx_callback */
