...
{
    if (I2S_TX_DMASx->CR & (1 << 19))neswitchbuf = 0;
    else neswitchbuf = 1;
...
int nes_sound_open(int samples_per_sync, int sample_rate)
{
    ...
    i2s_samplerate_set(sample_rate);            /* 设置采样率 */
    i2s_tx_dma_init((uint8_t *)saibuf1, (uint8_t *)saibuf2, 2 * APU_PCMBUF_SIZE);    /* 配置TX DMA */
    i2s_tx_callback = nes_sai_dma_tx_callback;  /* 回调函数指nes_sai_dma_tx_callback */
