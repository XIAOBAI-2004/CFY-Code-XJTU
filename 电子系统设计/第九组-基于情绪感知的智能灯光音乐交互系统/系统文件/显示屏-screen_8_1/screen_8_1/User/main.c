 ...
 * 修改说明
 * V1.1 20221227
 * 1, 修改system_files_erase函数为全片擦除方式,提高速度/避免出错!
 * 2, 修改norflash.c的norflash_read函数, 添加OS访问支持
 *
 ****************************************************************************************************
 */
...
//#define VOICE_TASK_PRIO        6       /* 任务优先级（建议值） */
//#define VOICE_STK_SIZE         128     /* 堆栈大小 */

//__ALIGNED_8 static OS_STK VOICE_TASK_STK[VOICE_STK_SIZE];  /* 任务堆栈,8字节对齐 */               
//										
...
uint8_t system_exsram_test(uint16_t x, uint16_t y, uint8_t fsize)
{
    ...
    for (i = 0; i < 1024 * 1024; i += 1024)
    {
        ...
        lcd_show_xnum(x + 15 * (fsize / 2), y, (uint16_t)(temp - sval + 1), 4, fsize, 0, WHITE); /* 显示内存容量 */
    }
...
void system_init(void)
{
    ...
    my_mem_init(SRAMCCM);               /* 初始化内部CCM SRAM内存池 */
...
REINIT: /* 重新初始化 */
    ...
    if (st480mc_init())system_error_show(5, ypos + fsize * (j + 1), "ST480MC Error!", fsize); /* CH395Q检测 */
    else lcd_show_string(5 + okoffset, ypos + fsize * j++, lcddev.width, lcddev.height, fsize, "OK", g_point_color);
...
//        res = exfuns_get_free("2:", &dtsize, &dfsize);	/* 得到U盘剩余容量和总容量 */
//        TIM3->CR1 &= ~(1 << 0);     /* 关闭定时器3 */
//    }
//    else res = 0XFF;
...
void start_task(void *pdata)
{
    ...
    OSTaskCreateExt((void(*)(void *) )watch_task,               /* 任务函数 */
     ...
     // 创建事件控制块用于语音消息通知
    //voice_event = OS_EVENT_CREATE("Voice Event");
...
void main_task(void *pdata)
{
    ...
    while(1)
    {
        uint8_t play_result = audio_play();  /* 循环调用播放器 */
        
        system_task_return = 0;
        delay_ms(10);
        
        if (++tcnt >= OS_TICKS_PER_SEC) {
            tcnt = 0;
            spb_stabar_msg_show(0);
        }
    }
}
...
void watch_task(void *pdata)
{
    ...
    while (1)
    {
        ...
        else if (tpad_scan(0))      /* TPAD按下了一次,此函数执行,至少需要15ms */
        {
            /********************* 新增代码开始 *********************/
            /* 仅在非文件浏览界面处理TPAD返回 */
//            if (g_current_ui == 1) { // 播放界面
//        if (g_player_state == PLAYER_PLAYING) {
//            // 保持播放返回文件浏览
//            g_current_ui = 0;
//            audio_filelist(&g_audiodev);
//        } else if (g_player_state == PLAYER_PAUSED) {
//            audio_stop();
//            g_current_ui = 0;
//        }
  }
