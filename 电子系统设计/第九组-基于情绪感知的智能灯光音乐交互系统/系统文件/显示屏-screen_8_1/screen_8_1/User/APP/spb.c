...
uint8_t *const spb_bkpic_path_tbl[3] =
{
    "1:/SYSTEM/SPB/BACKPIC/600224.jpg",
    "1:/SYSTEM/SPB/BACKPIC/800364.jpg",
...
uint8_t spb_init(uint8_t mode)
{
    ...
    if(lcddev.width == 480)     /* 对于800*480屏幕,只有一页,16个图标都在这一页里面, 无左右余量 */
    {
        picinfo.lcdwidth = spbdev.spbwidth * 1; /* 800*480, 只有屏幕宽 */
        spbdev.spbahwidth = 0;                  /* 800*480 无余量 */

    }
    else
    {
        picinfo.lcdwidth = spbdev.spbwidth * SPB_PAGE_NUM + spbdev.spbahwidth * 2;  /* 非800*480, 直接用宽图片 */
    ...
    g_sramlcdbuf = gui_memex_malloc((uint32_t)spbdev.spbheight * picinfo.lcdwidth * 2); /* 申请帧缓存大小 */

    if (g_sramlcdbuf == NULL)return 1; /* 错误 */

    res = piclib_ai_load_picfile((char *)spb_bkpic_path_tbl[lcdtype], 0, 0, picinfo.lcdwidth, spbdev.spbheight, 0); /* 加载背景图片 */
...
uint8_t spb_move_chk(void)
{
    ...
    uint8_t skips = 5;      /* 默认2.8寸屏, 每次跳5个像素 */

    tp_dev.scan(0);         /* 扫描 */

    if (lcddev.width == 320)    /* 对3.5寸屏来说, 每次跳过10个像素 */
    {
         skips = 10;
    }
