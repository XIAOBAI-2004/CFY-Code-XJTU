...
uint8_t app_system_file_check(uint8_t *diskx)
{
    ...
    while (1)
    {
        ...
        if (rval != 3 + 3 * SPB_ICOS_NUM)break;
...
void app_getstm32_sn(uint32_t *sn0, uint32_t *sn1, uint32_t *sn2)
{
    ...
    *sn1 = *(volatile uint32_t *)(0x1FFF7A14);
    *sn2 = *(volatile uint32_t *)(0x1FFF7A18);
}
...
uint8_t app_system_parameter_init(void)
{
    ...
    tim8_ch3n_pwm_init(100, 168 - 1);           /* 初始化TIM8_CH3N PWM输出,作为LCD的背光控制 */
