...
void video_audio_dma_callback(void)
{
    ...
    {
        I2S_TX_DMASx->M0AR = (uint32_t)videodev.saibuf[videodev.saiplaybuf]; /* 指向下一个buf */
    }
    else
    {
        I2S_TX_DMASx->M1AR = (uint32_t)videodev.saibuf[videodev.saiplaybuf]; /* 指向下一个buf */
    }
}
...
uint8_t video_play_mjpeg(void)
{
    ...
    while (res == 0)
    {
        ...
        if (res == 0)
        {
            ...
            tim6_int_init(g_avix.SecPerFrame / 100 - 1, 8400 - 1);  /* 10Khz计数频率,加1是100us,用于视频帧间隔控制 */
            ...
            res = mjpegdec_init((lcddev.width - g_avix.Width) / 2, yoff + (videoheight - g_avix.Height) / 2); /* JPG解码初始化 */
            ...
            if (g_avix.SampleRate)    /* 有音频信息,才初始化 */
            {
                ...
                i2s_tx_dma_init(videodev.saibuf[1], videodev.saibuf[2], g_avix.AudioBufSize / 2);   /* 配置DMA */
                i2s_tx_callback = video_audio_dma_callback; /* 回调函数指向sai_DMA_Callback */
                videodev.saiplaybuf = 0;
                saisavebuf = 0;
                i2s_play_start();   /* 开启sai播放 */
            }
