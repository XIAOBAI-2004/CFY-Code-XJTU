...
uint8_t *g_vubuf;           /* vu用buf,直接指向p_i2s_recbuf1/p_i2s_recbuf2 */
...
void rec_i2s_dma_rx_callback(void)
{
    ...
    {
        g_vubuf = p_i2s_recbuf1;    /* g_vubuf指向p_i2s_recbuf1 */
    ...
    {
        g_vubuf = p_i2s_recbuf2;    /* g_vubuf指向p_i2s_recbuf2 */

        if (g_rec_sta == 0X80)      /* 录音模式,非暂停,则写文件 */
        {
            recoder_i2s_fifo_write(p_i2s_recbuf2);  /* p_i2s_recbuf1 写入FIFO */
        }
    }
...
void recorder_enter_rec_mode(void)
{
    ...
    i2s_init(0, 2, 0, 0);           /* 飞利浦标准,主机发送,时钟低电平有效,16位帧长度 */
    i2sext_init(0, 1, 0, 0);        /* 飞利浦标准,从机接收,时钟低电平有效,16位帧长度 */

    i2s_tx_dma_init((uint8_t *)&i2splaybuf[0], (uint8_t *)&i2splaybuf[1], 1);       /* 配置TX DMA */
    I2S_TX_DMASx->CR &= ~(1 << 4);  /* I2S TX DMA数据流关闭传输完成中断(这里不用中断送数据) */
    i2sext_rx_dma_init(p_i2s_recbuf1, p_i2s_recbuf2, REC_I2S_RX_DMA_BUF_SIZE / 2);  /* 配置RX DMA */

    /* 降低I2S DMA中断的优先级,让USB优先级可以打断DMA中断优先级 */
    sys_nvic_init(1, 0, I2S_TX_DMASx_IRQn, 2);      /* 抢占1，子优先级0，组2 */
    sys_nvic_init(1, 1, I2SEXT_RX_DMASx_IRQn, 2);   /* 抢占1，子优先级1，组2 */
...
void recorder_new_pathname(uint8_t *pname)
{
    ...
    else if (gui_phy.memdevflag & (1 << 2))sprintf((char *)pname, "2:RECORDER/REC%04d%02d%02d%02d%02d%02d.wav", calendar.year, calendar.month, calendar.date, calendar.hour, calendar.min, calendar.sec);   /* SD卡不存在,则保存在U盘 */
}
...
uint8_t recorder_play(void)
{
    ...
    {
        p_i2s_recfifo_buf[i] = gui_memin_malloc(REC_I2S_RX_DMA_BUF_SIZE);   /* I2S录音FIFO内存申请 */
    ...
    p_i2s_recbuf1 = gui_memin_malloc(REC_I2S_RX_DMA_BUF_SIZE);  /* I2S录音内存1申请 */
    p_i2s_recbuf2 = gui_memin_malloc(REC_I2S_RX_DMA_BUF_SIZE);  /* I2S录音内存2申请 */
    ...
    if (!p_i2s_recbuf2 || !f_rec || !wavhead || !buf || !pset_bkctbl)rval = 1;
    else
    {
        /* 加载字体 */
    ...
    if (rval == 0)
    {
        ...
        while (rval == 0)
        {
            ...
            if (recoder_i2s_fifo_read(&pdatabuf) && (g_rec_sta & 0X80)) /* 读取一次数据,读到数据了,写入文件 */
            {
                res = f_write(f_rec, pdatabuf, REC_I2S_RX_DMA_BUF_SIZE, (UINT *)&bread);    /* 写入文件 */
                ...
                g_wav_size += REC_I2S_RX_DMA_BUF_SIZE;
            }
            else
            {
                timecnt++;
    ...
    for (i = 0; i < REC_I2S_RX_FIFO_SIZE; i++)
    {
        gui_memin_free(p_i2s_recfifo_buf[i]);   /* I2S录音FIFO内存释放 */
    }
    
    gui_memin_free(p_i2s_recbuf1);
    gui_memin_free(p_i2s_recbuf2);
