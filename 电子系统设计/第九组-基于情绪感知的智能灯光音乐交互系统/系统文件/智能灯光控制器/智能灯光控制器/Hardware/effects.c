...
static EffectType current_effect = EFFECT_HAPPY;
static EffectType target_effect = EFFECT_HAPPY;
...
// 惊喜效果 - 金粉粒子爆炸
static void Effect_Surprise(void) {
...
static void Effect_Complex(void) {
    static EffectType sub_effect = EFFECT_HAPPY;
    ...
    // 根据当前子效果调用对应函数
    switch(sub_effect) {
        case EFFECT_HAPPY:  Effect_Happy(); break;
        case EFFECT_ANGRY:  Effect_Angry(); break;
...
// 憎恶效果 - 暗绿爆炸流水
static void Effect_Disgust(void) {
    static float flow_pos = 0;
    static uint8_t explosion_index = 0;
    static uint8_t flash_counter = 0;     // 全局闪烁计时器
    static uint8_t flash_state = 0;       // 全局闪烁状态
    static uint8_t explosion_counter = 0; // 爆炸点闪烁计数器
    const uint8_t flow_speed = LED_NUM / 60; // 3秒循环(60灯)
    
    // 全局闪烁控制（每1.5秒触发一次）
    if(++flash_counter >= 30) {          // 30帧 * 50ms = 1.5秒
        flash_counter = 0;
        flash_state = 3;                 // 持续3帧闪烁
    }
    ...
    for(uint16_t i = 0; i < LED_NUM; i++) {
        // 原始波浪效果
        float val = 0.3f + 0.7f * sinf((i + flow_pos) * 0.1f);
        ...
        // 全局闪烁效果（黄绿色）
        if(flash_state > 0) {
            float flash_intensity = flash_state * 0.3f; // 渐弱闪烁
            base_color.r = (uint8_t)(base_color.r + 100 * flash_intensity);
            base_color.g = (uint8_t)(base_color.g + 50 * flash_intensity);
            // 防止颜色溢出
            base_color.r = base_color.r > 255 ? 255 : base_color.r;
            base_color.g = base_color.g > 255 ? 255 : base_color.g;
        }
        
        WS2812_SetColor(i, base_color);
    }
    
    // 随机位置触发爆炸（增强闪烁）
    if(rand() % 15 == 0) {
        explosion_index = (explosion_index + 1) % 5;
        uint16_t pos = rand() % LED_NUM;
        float radius = 5 + rand() % 8;
        
        for(int i = pos - radius; i <= pos + radius; i++) {
            if(i >= 0 && i < LED_NUM) {
                float dist = fabsf(i - pos) / radius;
                float intensity = powf(1 - dist, 2);
                ...
                // 爆炸点闪烁（红黄交替）
                RGB_Color explosion_color;
                if(explosion_counter % 4 < 2) { // 闪烁频率控制
                    explosion_color = (RGB_Color){
                        (uint8_t)(200 * intensity),  // 黄色G分量
                        (uint8_t)(255 * intensity),  // 红色R分量
                        (uint8_t)(50 * intensity)    // 橙色B分量
                    };
                } else {
                    explosion_color = (RGB_Color){
                ...
                RGB_Color final_color = {
                    (uint8_t)fminf(base_color.g + explosion_color.g, 255),
                    (uint8_t)fminf(base_color.r + explosion_color.r, 255),
                    (uint8_t)fminf(base_color.b + explosion_color.b, 255)
                };
                
                WS2812_SetColor(i, final_color);
            }
        }
        explosion_counter++; // 更新爆炸点闪烁计数器
    }
    
    // 更新全局闪烁状态
    if(flash_state > 0) flash_state--;
...
// 中立效果 - 双色呼吸灯
static void Effect_Neutral(void) {
    static float breath = 0;
    breath += 0.015f;
    
    // 暖白(2700K)到黄色(3500K)渐变
    float ratio = (sinf(breath) + 1) / 2;
    uint8_t warm = 180 + ratio * 50;   // 暖白分量
    uint8_t yellow = 220 + ratio * 30; // 黄色分量
    
    // 呼吸亮度曲线
    float brightness = (sinf(breath * 1.2f) + 1) / 2 * 0.8f + 0.2f;
    
    RGB_Color c = {
        (uint8_t)(warm * brightness),   // G
        (uint8_t)(yellow * brightness), // R
        (uint8_t)(50 * brightness)      // B
    };
    WS2812_SetAll(c);
    WS2812_Update();
    delay_ms(50);
...
// 恐惧效果 - 紫红闪爆
static void Effect_Fear(void) {
    static float hue_shift = 0;
    static uint8_t flash_counter = 0;
    
    // 紫红渐变背景
    for(uint16_t i = 0; i < LED_NUM; i++) {
        float hue = 300 + 20 * sinf(hue_shift + i * 0.05f);
        RGB_Color c = HSVtoRGB(hue, 0.9f, 0.6f);
        WS2812_SetColor(i, c);
    }
    hue_shift += 0.5f;
    
    // 随机白色闪爆
    if(flash_counter == 0 && rand() % 15 == 0) {
        flash_counter = 8;
        uint16_t center = rand() % LED_NUM;
        uint8_t radius = 5 + rand() % 10;
        
        for(int i = center - radius; i <= center + radius; i++) {
            if(i >= 0 && i < LED_NUM) {
                float dist = fabsf(i - center) / radius;
                float val = powf(1 - dist, 2);
                WS2812_SetColor(i, (RGB_Color){
                    (uint8_t)(255 * val),
                    (uint8_t)(255 * val),
                    (uint8_t)(255 * val)
                });
...
void Effects_Run(void) {
    ...
    switch(current_effect) {
		case EFFECT_HAPPY:     Effect_Happy();     break; // 喜
        case EFFECT_ANGRY:   Effect_Angry();   break; // 怒
		...
		case EFFECT_DISGUST:  Effect_Disgust();  break;
        case EFFECT_FEAR:     Effect_Fear();     break;
        case EFFECT_SURPRISE:Effect_Surprise(); break;
        case EFFECT_NEUTRAL: Effect_Neutral();  break;
        case EFFECT_FLOW: {
            // 流水效果
            WS2812_Clear();
